...
namespace ToDoList.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public ObservableCollection<List6> Items { get; set; }



        private Dictionary<DateTimeOffset, List<List6>> ListsOnDays;
        ...
        {
            this.ListsOnDays = new Dictionary<DateTimeOffset, List<List6>>();
            this.Items = new ObservableCollection<List6>();
            this.Content = new FirstViewModel();
        }

        private void InitToDoList()
        {
            var ListsOnDays = new Dictionary<DateTimeOffset, List<List6>>();
            ListsOnDays.Add(this.date, new List<List6>());
            this.ListsOnDays = ListsOnDays;
        }

        public void AppendAction(DateTimeOffset date, List6 item)
        {
            if (!this.ListsOnDays.ContainsKey(date))
            {
                this.ListsOnDays.Add(date, new List<List6>());
            }
        ...
        public void SaveChanges()
        {
            if (this.Title != "")
            {
                ...
                {
                    this.AppendAction(this.Date, new List6(this.Title, this.Description));
                }
                this.ChangeView();
            }
        }

        public void DeleteItem(List6 item)
        {
            this.ListsOnDays[date].Remove(item);
            this.ChangeObservableCollection(date);
        }

        public void ViewExisting(List6 item)
        {
